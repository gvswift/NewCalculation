 let baseIDRate, baseETRate, baseCSCRate, baseVATRate, baseSurcharge, baseVATBase = 0;
            ...
            function resetSpecs() {
                ...
                specsInput.forEach((b) => { b.value = ""; });
                hideDuties();
            }

            document.getElementById("make").addEventListener("keyup", e => {
                document.getElementById("make").value = document.getElementById("make").value.toUpperCase();
            });

            document.getElementById("make").addEventListener("keydown", e => {
                if(e.key === 'Enter'){ document.getElementById("model").focus(); }
            });
            ...
            document.getElementById("model").addEventListener("keydown", e => {
                if(e.key === "Enter"){ document.getElementById("chassis").focus(); }
            });

            document.getElementById("chassis").addEventListener("keyup", e => { 
                document.getElementById("chassis").value = document.getElementById("chassis").value.toUpperCase();
            });

            document.getElementById("chassis").addEventListener("keydown", e => {
                if(e.key === "Enter"){ document.getElementById("year").focus(); }
            });
            ...
            function resetAllDuties(){
                ...
                removeDuties.forEach((e) => { e.innerHTML = ""; });  
            ...
            function calculation(){
                ...
                switch(hd.value){
                    case "8701":
                        ...
                        document.getElementById("weight").disabled = true;
                        baseSurcharge = 0;
                        document.getElementById("vs-amount").innerText = baseSurcharge.toFixed(2);
                        if(source.value === "electric" || source.value === "hybrid"){
                            baseIDRate = 5.00;
                            baseETRate = 35.00;
                            baseCSCRate = 6.00;
                            baseVATBase = (cifec * baseIDRate/100) + (cifec * baseETRate/100) + (cifec * baseCSCRate/100) + baseSurcharge + cifec;
                            baseVATRate = 16.00;
                        } else {
                            baseIDRate = 5.00;
                            baseETRate = 55.00;
                    ...
                    case "8702":
                        ...
                        if(source.value === "electric" || source.value === "hybrid"){
                            ...
                            baseETRate = 35.00;
                            baseCSCRate = 6.00;
                            baseVATBase = (baseIDRate * cifec/100) + (baseETRate * cifec/100) + (baseCSCRate * cifec/100) + baseSurcharge + cifec;
                            baseVATRate = 16.00;
                        } else {
                            baseIDRate = 10.00;
                            baseETRate = 45.00;
                    ...
                    case "8703":
                        ...
                        if(source.value === "gas" || source.value === "diesel" || source.value === "hybrid") {
                            ...
                            let range_factor = rangeFactors8703[r].factor; 
                    ...
                    case "8704":
                        ...
                        age_factor = ageFactors8702_8704[a].factor;
                        let tonnage_factor = tonnageFactors8704[t].factor;
                        ...
                        if(source.value ==="electric" || source.value === "hybrid"){
                            ...
                            baseETRate = 30.00;
                            baseCSCRate = 6.00;
                            baseVATBase = (baseIDRate * cifec/100) + (baseETRate * cifec/100) + baseSurcharge + (baseCSCRate * cifec/100) + cifec;
                            baseVATRate = 16.00;
                        } else if (source.value === "other") {
                            baseIDRate = 10.00;
                            baseETRate = 60.00;
                    ...
                    case "8705":
                        if (option.value === "fire_fighting"){
                            ...
                            a = ageFactors8705.findLastIndex((vehicle) => vehicle.age <= Number(v_age.value));
                            c = cifMatrix8705.findIndex((matrixCif) => matrixCif.cif <= cifec);
                            age_factor = ageFactors8705[a].factor;
                ...
                document.getElementById("vb-amount").innerHTML = moneyfm.format(baseVATBase);
            ...
            function calculateDuties(){
                if(document.getElementById("hs").value === ""){
                    ...
                    document.getElementById("total-duties").innerText = moneyfm.format(totalDuties);
            ...
            document.getElementById("iduty-waiver").addEventListener("keydown", e => {
                if(e.key === "Enter"){
                    let idw = document.getElementById("iduty-waiver").value;
                    if(idw === ""){
                        document.getElementById("etax-waiver").focus();
                    } else if(Number(idw) < 0 || Number(idw) > 100){
                        alert("Invalid Data!");
                        document.getElementById("iduty-waiver").focus();
                        document.getElementById("iduty-waiver").value = "";
                    } else {
                        document.getElementById("etax-waiver").focus();
                    }
                }
            ...
            document.getElementById("etax-waiver").addEventListener("keydown", e => {
                if(e.key === "Enter"){
                    let etw = document.getElementById("etax-waiver").value;
                    if(etw === ""){
                        document.getElementById("csc-waiver").focus();
                    } else if(Number(etw) < 0 || Number(etw) > 100){
                        alert("Invalid Data!");
                        document.getElementById("etax-waiver").focus();
            ...
            document.getElementById("csc-waiver").addEventListener("keydown", e => {
                if(e.key === "Enter"){
                    let csw = document.getElementById("csc-waiver").value;
                    if(csw === ""){
                        document.getElementById("vs-waiver").focus();
                    } else if(Number(csw) < 0 || Number(csw) > 100){
                        alert("Invalid Data!");
                        document.getElementById("csc-waiver").focus();
            ...
            document.getElementById("vs-waiver").addEventListener("keydown", e => {
                if(e.key === "Enter"){
                    let vsw = document.getElementById("vs-waiver").value;
                    if(vsw === ""){
                        document.getElementById("vat-waiver").focus();
                    } else if(Number(vsw) < 0 || Number(vsw) > 100){
                        alert("Invalid Data!");
                        document.getElementById("vs-waiver").focus();
                        document.getElementById("vs-waiver").value = "";
                    } else {   
                        document.getElementById("vat-waiver").focus();
                    }
                }
            ...
            document.getElementById("vat-waiver").addEventListener("keydown", e => {
                if(e.key === "Enter"){
                    let vtw = document.getElementById("vat-waiver").value;
                    if(vtw === ""){
                        document.getElementById("iduty-waiver").focus();
                    } else if(Number(vtw) < 0 || Number(vtw) > 100){
                        alert("Invalid Data!");
                        document.getElementById("vat-waiver").focus();
            ...
            function apply_Waiver(){
                ...
                if(id_w != ""){
                    let IDWRate = baseIDRate * (100 - id_w) / 100;
                ...
                if(et_w != ""){
                    let ETWRate = baseETRate * (100 - et_w) / 100;
                ...
                if(csc_w != ""){
                    let CSCWRate = baseCSCRate * (100 - csc_w) / 100;
                ...
                if(hd.value === "8701"){
                    document.getElementById("vs-amount").innerHTML = "0.00";
                } else {
                    if(vs_w != ""){
                        let VSWAmount = baseSurcharge * (100 - vs_w) / 100;
                ...
                if(vat_w != ""){
                    let VATWRate = baseVATRate * (100 - vat_w) / 100;
